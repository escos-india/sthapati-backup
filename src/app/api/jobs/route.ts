...
export async function GET(req: Request) {
    try {
        ...
        const jobs = await JobModel.find(query)
            .sort({ createdAt: -1 })
            .populate("posted_by", "name image category")
            .lean();
...
export async function POST(req: Request) {
    try {
        ...
        // Check if user is allowed to post jobs (Non-Student, Non-Trade Professional ideally, but let's stick to "Job Posters" logic)
        // Logic: Students and Trade Professionals are "Job Seekers". Others are "Job Posters".
